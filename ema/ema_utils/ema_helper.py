...
def rollout(model, A, X_t_norm, mean, std, steps):
    ...
    model : GNet_EMA
    A : [N,N]
    X_t_norm : [N,F]  normalized features at initial time t
    mean, std : [1,1,F]
    steps : int
    
    Returns
    -------
    preds_denorm : list of length `steps`
        Each element is [N,3] positions (denormalized)
    """
    device = X_t_norm.device
    preds_denorm = []

    current_norm = X_t_norm  # normalized input

    for _ in range(steps):

        # 1) Predict next normalized features
        with torch.no_grad():
            pred_norm = model.rollout_step(A, current_norm)   # [N,F]

        # 2) Extract normalized coords
        coords_pred_norm = pred_norm[:, :3]  # [N,3]

        # 3) De-normalize coords for visualization
        coords_pred = coords_pred_norm * std[:, :, :3] + mean[:, :, :3]
        preds_denorm.append(coords_pred.cpu().numpy())

        # 4) Re-normalize full pred to feed next step
        pred_norm_full = pred_norm  # already normalized
        current_norm = pred_norm_full
