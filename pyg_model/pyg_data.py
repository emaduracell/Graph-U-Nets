import json
import os

import numpy as np
import torch

from tfrecord.torch.dataset import TFRecordDataset

from torch.nn import functional as F
from torch.utils.data import Dataset

import torch_geometric as pyg

from physicsnemo.datapipes.gnn.utils import load_json, save_json

"""
Every graph object has
- edge_attr
"""


class DeformingPlateDataset(Dataset):
    """
    In-memory MeshGraphNet Dataset for stationary mesh
    Notes:
        - This dataset prepares and processes the data available in MeshGraphNet's repo:
            https://github.com/deepmind/deepmind-research/tree/master/meshgraphnets
        - A single adj matrix is used for each transient simulation.
            Do not use with adaptive mesh or remeshing


    name: str
        name of the dataset: 'deforming_plate_train' | 'deforming_plate_test'
    data_dir: str | None
        where the TFRecord is stored
    split: str
        'train' or 'test'
    num_samples: int
        number of samples to load (trajectories)
    num_steps: int
        number of time steps to load for each traj
    noise_std: float
        std dev of noise
    length: int
        number of instances in the dataset, so num_samples * (num_steps - 1)
    torch_ds:
        TFRecordDataset initially, then the new dataset
    graphs: list[pyg.data.Data]
        graph object: the list should have dimension num_samples. The desired time step is obtained via
        __getitem__. The graph object just stores the static/time-invariant topology.
    cells: list[np.ndarray]
        tetrahedral mesh connectivity
    node_type: list[torch.Tensor]
        node type list
    moving_points_mask: list[torch.Tensor]
        mask of moving points
    object_points_mask: list[torch.Tensor]
        mask of actuator points
    clamped_points_mask: list[torch.Tensor]
        mask of constrained points
    mesh_pos: list[torch.Tensor]
        mesh poisition list
    edge_stats: dict[str, torch.Tensor]
        statistics of edge features (world or mesh)
    node_features: list[dict[str, torch.Tensor]]
        node features
    node_targets: list[dict[str, torch.Tensor]]
        node targets?
    node_stats: dict[str, torch.Tensor]
        statistics of node features
    """

    def __init__(self, name="dataset", data_dir=None, split="train", num_samples=1000, num_steps=400, noise_std=0.003):
        """
        Constructs an instance of DeformingPlateDataset.
        1. Define the schema per meta.json
        2. Create graphs, edges, node features, edge features
        3. Add noise
        4. Normalize and save stats about both edge and node features
        """
        self.name = name
        self.data_dir = data_dir
        self.split = split
        self.num_samples = num_samples
        self.num_steps = num_steps
        self.noise_std = noise_std
        self.length = num_samples * (num_steps - 1)

        print(f"Preparing the {split} dataset...")
        # create the graphs with edge features
        # Build TFRecordDataset from .tfrecord file
        tfrecord = os.path.join(data_dir, f"{split}.tfrecord")
        index = None  # or path to .index if you generated it
        # Define the schema per meta.json
        meta = json.load(open(os.path.join(data_dir, "meta.json")))
        description = {k: "byte" for k in meta["field_names"]}  # raw bytes
        self.torch_ds = TFRecordDataset(tfrecord, index, description,
                                        transform=lambda rec: self._decode_record(rec, meta))
        self.graphs, self.cells, self.node_type = [], [], []
        (self.moving_points_mask, self.object_points_mask, self.clamped_points_mask) = [], [], []
        noise_mask = []
        self.mesh_pos = []
        # Before this loop, torch_ds is actually the TFRecord dataset
        # Create all graphs, node type and noise mask
        for idx, rec in enumerate(self.torch_ds):
            if idx >= num_samples:
                break
            # create dict with (key, v)
            data_np = {k: v[:num_steps] for k, v in rec.items()}
            # given the first element of "cells" dict (since the adjacency matrix of the mesh doesn't change) create the
            # adjacency matrix, generate source nodes and destination nodes
            src, dst = self.cell_to_adj(data_np["cells"][0])
            # Create the graph with all edges
            graph = self.create_graph(src, dst, dtype=torch.int32)
            # Add edge features to graph (relative displacement, displacement norm)
            # FIXME: deleted for Graph-U-Net: we don't have edge features
            # graph = self.add_edge_features(graph, data_np["mesh_pos"][0])
            # Add graph to graphs set
            self.graphs.append(graph)
            # Create tensor from node type
            node_type = torch.tensor(data_np["node_type"][0], dtype=torch.uint8)
            self.node_type.append(self._one_hot_encode(node_type))
            # FIXME: deleted for Graph-U-Net: we don't have noise yet
            # noise_mask.append(torch.eq(node_type, torch.zeros_like(node_type)))

            if self.split != "train":
                self.mesh_pos.append(torch.tensor(data_np["mesh_pos"][0]))
                self.cells.append(data_np["cells"][0])
                moving_points_mask, object_points_mask, clamped_points_mask = (self._get_rollout_mask(node_type))
                self.moving_points_mask.append(moving_points_mask)
                self.object_points_mask.append(object_points_mask)
                self.clamped_points_mask.append(clamped_points_mask)

        # After creating all graphs, node type and noise mask compute or load edge data stats
        # FIXME: deleted for Graph-U-Net: we don't have edge features
        # if self.split == "train":
        #     self.edge_stats = self._get_edge_stats()
        # else:
        #     self.edge_stats = load_json("edge_stats.json")
        # FIXME: deleted for Graph-U-Net: we don't have edge features
        # normalize edge features
        # for idx in range(num_samples):
        #     self.graphs[idx].edge_attr = self.normalize_edge(self.graphs[idx], self.edge_stats["edge_mean"],
        #                                                      self.edge_stats["edge_std"])

        # create the node features
        self.node_features, self.node_targets = [], []
        for idx, rec in enumerate(self.torch_ds):
            if idx >= num_samples:
                break
            data_np = {k: v[:num_steps] for k, v in rec.items()}
            features, targets = {}, {}
            # Discards the last time step
            features["world_pos"] = self._drop_last(data_np["world_pos"])  # Shape: (num_steps-1, num_nodes, num_features)
            # Calculates pos[t+1] - pos[t].
            targets["velocity"] = self._push_forward_diff(data_np["world_pos"])  # Shape: (num_steps-1, num_nodes, num_features)
            # Discards the first time step.
            targets["stress"] = self._push_forward(data_np["stress"])  # Shape: (num_steps-1, num_nodes, num_features)

            # add noise: FIXME: DELETED SINCE WE'RE NOT INTERESTED YET IN IT
            # if (split == "train"):  # TODO: noise has to be added at each iteration during training
            #     features["world_pos"], targets["velocity"] = self._add_noise(
            #         features["world_pos"],
            #         targets["velocity"],
            #         self.noise_std,
            #         noise_mask[idx],
            #     )
            self.node_features.append(features)
            self.node_targets.append(targets)

        # compute or load node data stats
        if self.split == "train":
            self.node_stats = self._get_node_stats()
        else:
            self.node_stats = load_json("node_stats.json")

        # normalize node features
        for idx in range(num_samples):
            # print("="*60+"\nvelocity\n"+"="*60)
            self.node_targets[idx]["velocity"] = self.normalize_node(self.node_targets[idx]["velocity"],
                                                                     self.node_stats["velocity_mean"],
                                                                     self.node_stats["velocity_std"])
            # print("=" * 60 + "\nstress\n" + "=" * 60)
            self.node_targets[idx]["stress"] = self.normalize_node(self.node_targets[idx]["stress"],
                                                                   self.node_stats["stress_mean"],
                                                                   self.node_stats["stress_std"])
            self.node_targets[idx]["mesh_pos"] = self.normalize_node(self.node_targets[idx]["mesh_pos"],
                                                                   self.node_stats["mesh_pos_mean"],
                                                                   self.node_stats["mesh_pos_std"])
            self.node_targets[idx]["world_pos"] = self.normalize_node(self.node_targets[idx]["world_pos"],
                                                                     self.node_stats["world_pos_mean"],
                                                                     self.node_stats["world_pos_std"])

    def __getitem__(self, idx):
        """
        Gets item specified by idx of the dataset.
        returns one time step of one sample, re-using that sample’s graph topology but with time-dependent features.

        :param idx: int
            index of dataset
        :return
            train mode: graph
                graph
            test mode: (graph, cells, moving_points_mask, object_points_mask, clamped_points_mask)
                graphs and cells and mask
        """
        gidx = idx // (self.num_steps - 1)  # graph index
        tidx = idx % (self.num_steps - 1)  # time step index
        graph = self.graphs[gidx].clone()
        node_features = self.node_type[gidx].float()
        node_targets = torch.cat((self.node_targets[gidx]["velocity"][tidx],
                                  self.node_targets[gidx]["stress"][tidx]), dim=-1)
        graph.x = node_features
        graph.y = node_targets
        graph.world_pos = self.node_features[gidx]["world_pos"][tidx]
        if self.split == "train":
            return graph
        else:
            graph.mesh_pos = self.mesh_pos[gidx]
            cells = torch.as_tensor(self.cells[gidx])
            moving_points_mask = self.moving_points_mask[gidx]
            object_points_mask = self.object_points_mask[gidx]
            clamped_points_mask = self.clamped_points_mask[gidx]

            return (graph, cells, moving_points_mask, object_points_mask, clamped_points_mask)

    def __len__(self):
        return self.length

    def _get_edge_stats(self):
        """
        Computes edge statistics {"edge_mean", "edge_meansqr"} and saves them in a json

        :return stats
            edge statistics
        """
        stats = {
            "edge_mean": 0,
            "edge_meansqr": 0,
        }
        for i in range(self.num_samples):
            stats["edge_mean"] += (torch.mean(self.graphs[i].edge_attr, dim=0) / self.num_samples)
            stats["edge_meansqr"] += (
                    torch.mean(torch.square(self.graphs[i].edge_attr), dim=0)
                    / self.num_samples
            )
        stats["edge_std"] = torch.sqrt(stats["edge_meansqr"] - torch.square(stats["edge_mean"]))
        stats.pop("edge_meansqr")

        # save to file
        save_json(stats, "edge_stats.json")
        return stats

    def _get_node_stats(self):
        """
        Computes the following stats about nodes and saves them in a json:
        stats = {
            "velocity_mean",
            "velocity_meansqr",
            "stress_mean",
            "stress_meansqr",
        }
        """

        stats = {
            "velocity_mean": 0,
            "velocity_meansqr": 0,
            "stress_mean": 0,
            "stress_meansqr": 0,
        }
        for i in range(self.num_samples):
            stats["velocity_mean"] += (torch.mean(self.node_targets[i]["velocity"], dim=(0, 1))
                                       / self.num_samples)
            stats["velocity_meansqr"] += (torch.mean(torch.square(self.node_targets[i]["velocity"]), dim=(0, 1))
                                          / self.num_samples)
            stats["stress_mean"] += (
                    torch.mean(self.node_targets[i]["stress"], dim=(0, 1))
                    / self.num_samples
            )
            stats["stress_meansqr"] += (
                    torch.mean(torch.square(self.node_targets[i]["stress"]), dim=(0, 1))
                    / self.num_samples
            )
        stats["velocity_std"] = torch.sqrt(
            stats["velocity_meansqr"] - torch.square(stats["velocity_mean"])
        )
        stats["stress_std"] = torch.sqrt(
            stats["stress_meansqr"] - torch.square(stats["stress_mean"])
        )
        stats.pop("velocity_meansqr")
        stats.pop("stress_meansqr")

        # save to file
        save_json(stats, "node_stats.json")
        return stats

    @staticmethod
    def cell_to_adj(cells):
        """
        Creates adjacency matrix in COO format from mesh cells (tetrahedra --> 6 edges).
        (num_cells, 4) Each row is one tetrahedron (a 3D simplex with 4 vertices)

        :param cells:
            a tetrahedron mesh cell
        :return (src, dst)
            src: List
                list of source nodes, that is, nodes that have one arrow going out
            dst: List
                list of destination nodes, that is, nodes that have one arrow going in
        """
        num_cells = np.shape(cells)[0]
        # For each tetrahedron i, generate all 6 edges.
        edge_indices = [(0, 1), (0, 2), (0, 3), (1, 2), (1, 3), (2, 3)]
        src = [cells[i][a] for i in range(num_cells) for a, b in edge_indices]
        dst = [cells[i][b] for i in range(num_cells) for a, b in edge_indices]
        return src, dst

    @staticmethod
    def create_graph(src, dst, dtype=torch.int32):
        """
        Creates a PyG graph from an adj matrix in COO format.
        torch.int32 can handle graphs with up to (2**31) - 1 nodes or edges.

        :param src: List
            list of source nodes
        :param dst: List
            list of destination nodes

        :return graph
            created graph in coo format
        """
        edges = torch.stack([torch.tensor(src), torch.tensor(dst)], dim=0).long()
        edges = pyg.utils.to_undirected(edges)
        # Row-wise sorts edge_index and removes its duplicated entries. Duplicate entries in edge_attr are merged by
        # scattering them together according to the given reduce option. Default="sum"
        edges = pyg.utils.coalesce(edges)
        # See https://pytorch-geometric.readthedocs.io/en/latest/modules/utils.html#torch_geometric.utils.coalesce
        if isinstance(edges, tuple):
            edges = edges[0]
        graph = pyg.data.Data(edge_index=edges)
        return graph

    @staticmethod
    def add_edge_features(graph, pos):
        """
        Adds relative displacement & displacement norm as edge features

        :param graph
            input graph PyG object
        :param pos
            mesh position feature tensor

        :return graph
            graph with new edge features
        """
        row, col = graph.edge_index
        disp = torch.tensor(pos[row] - pos[col])
        disp_norm = torch.linalg.norm(disp, dim=-1, keepdim=True)
        graph.edge_attr = torch.cat((disp, disp_norm), dim=1)
        return graph

    @staticmethod
    def normalize_node(invar, mu, std):
        """
        Normalizes a tensor. Used for velocity and stress.

        Args:
            invar:
                tensor to normalize
            mu: same size of feature, e.g. for velocity it's 3 and for stress 1
                mean
            std: same size of feature, e.g. for velocity it's 3 and for stress 1
                std dev
        """
        if (invar.size()[-1] != mu.size()[-1]) or (invar.size()[-1] != std.size()[-1]):
            raise AssertionError("input and stats must have the same size")

        return (invar - mu.expand(invar.size())) / std.expand(invar.size())

    # @staticmethod
    # def normalize_node(invar, mu, std):
    #     """
    #     Normalizes a tensor. Used for velocity and stress.
    #
    #     Args:
    #         invar:
    #
    #         mu:
    #             mean
    #         std:
    #             std dev
    #     """
    #
    #     # Input shapes
    #     print(f"invar.shape: {invar.shape}")
    #     print(f"mu.shape:    {mu.shape}")
    #     print(f"std.shape:   {std.shape}")
    #
    #     # Input contents
    #     print(f"\nmu (mean):   {mu}")
    #     print(f"std:         {std}")
    #
    #     # Show a slice of invar (first timestep, first few nodes)
    #     print(f"\ninvar sample [0, :5, :] (first timestep, first 5 nodes):")
    #     print(invar[0, :5, :])
    #
    #     if (invar.size()[-1] != mu.size()[-1]) or (invar.size()[-1] != std.size()[-1]):
    #         raise AssertionError("input and stats must have the same size")
    #
    #     # Expanded shapes
    #     mu_expanded = mu.expand(invar.size())
    #     std_expanded = std.expand(invar.size())
    #     print(f"\nmu.expand(invar.size()).shape:  {mu_expanded.shape}")
    #     print(f"std.expand(invar.size()).shape: {std_expanded.shape}")
    #
    #     # Result
    #     result = (invar - mu_expanded) / std_expanded
    #
    #     print(f"\nresult.shape: {result.shape}")
    #     print(f"result sample [0, :5, :] (first timestep, first 5 nodes):")
    #     print(result[0, :5, :])
    #
    #     # Stats on result to verify normalization
    #     print(f"\nResult statistics (should be ~0 mean, ~1 std per component):")
    #     print(f"  result.mean(dim=(0,1)): {result.mean(dim=(0, 1))}")
    #     print(f"  result.std(dim=(0,1)):  {result.std(dim=(0, 1))}")
    #     print("=" * 60)
    #
    #     return result

    # FIXME RE-ESTABLISH AFTER
    @staticmethod
    def normalize_edge(graph, mu, std):
        """normalizes a tensor"""
        if (graph.edge_attr.size()[-1] != mu.size()[-1]
                or graph.edge_attr.size()[-1] != std.size()[-1]):
            raise AssertionError("Graph edge data must be same size as stats.")
        return (graph.edge_attr - mu) / std

    @staticmethod
    def denormalize(invar, mu, std):
        """denormalizes a tensor"""
        denormalized_invar = invar * std + mu
        return denormalized_invar

    @staticmethod
    def _one_hot_encode(node_type):
        """
        One hot encode node type feature.
        """
        # node_type: tensor of shape (...), values in {0, 1, 3}
        node_type = torch.squeeze(node_type, dim=-1)
        # Map 0 -> 0, 1 -> 1, 3 -> 2
        mapping = {0: 0, 1: 1, 3: 2}
        mapped = torch.full_like(node_type, fill_value=-1)
        for k, v in mapping.items():
            mapped[node_type == k] = v
        if (mapped == -1).any():
            raise ValueError("node_type contains values outside of {0, 1, 3}")
        node_type = F.one_hot(mapped.long(), num_classes=3)
        return node_type

    @staticmethod
    def _drop_last(invar):
        return torch.tensor(invar[0:-1], dtype=torch.float)

    @staticmethod
    def _push_forward(invar):
        return torch.tensor(invar[1:], dtype=torch.float)

    @staticmethod
    def _push_forward_diff(invar):
        return torch.tensor(invar[1:] - invar[0:-1], dtype=torch.float)

    @staticmethod
    def _get_rollout_mask(node_type):
        """
        Gets mask for
        1) moving points (node_type == 0)
        2) object points (node_type == 1)
        3) clamped points (node_type == 3)
        """
        moving_points_mask = torch.eq(node_type, torch.zeros_like(node_type))
        object_points_mask = torch.eq(node_type, torch.zeros_like(node_type) + 1)
        clamped_points_mask = torch.eq(node_type, torch.zeros_like(node_type) + 3)
        return moving_points_mask, object_points_mask, clamped_points_mask

    @staticmethod
    def _add_noise(features, targets, noise_std, noise_mask):
        noise = torch.normal(mean=0, std=noise_std, size=features.size())
        noise_mask = noise_mask.expand(features.size()[0], -1, 3)
        noise = torch.where(noise_mask, noise, torch.zeros_like(noise))
        features += noise
        targets -= noise
        return features, targets

    def _decode_record(self, rec_bytes, meta):
        out = {}
        for k, v in rec_bytes.items():
            dtype = meta["features"][k]["dtype"]
            shape = meta["features"][k]["shape"]
            arr = np.frombuffer(v, dtype=getattr(np, dtype))
            arr = arr.reshape(shape)
            if meta["features"][k]["type"] == "static":
                arr = np.tile(arr, (meta["trajectory_length"], 1, 1))
            out[k] = arr
        return out
