...
class DeformingPlateDataset(Dataset):
    ...
    def __init__(self, name="dataset", data_dir=None, split="train", num_samples=1000, num_steps=400, noise_std=0.003):
        ...
        # After creating all graphs, node type and noise mask
        ...
        for i in range(num_samples):
            self.graphs[i].edge_attr = self.normalize_edge(self.graphs[i], self.edge_stats["edge_mean"],
        ...
        for i in range(num_samples):
            self.node_targets[i]["velocity"] = self.normalize_node(self.node_targets[i]["velocity"],
                self.node_stats["velocity_mean"], self.node_stats["velocity_std"])
            self.node_targets[i]["stress"] = self.normalize_node(self.node_targets[i]["stress"],
                self.node_stats["stress_mean"], self.node_stats["stress_std"],)
