...
class EGNN(nn.Module):
    ...
    def __init__(self, dim, edge_dim=0, m_dim=16, fourier_features=0, num_nearest_neighbors=0, dropout=0.0,
                 init_eps=1e-3, norm_feats=False, norm_coors=False, norm_coors_scale_init=1e-2,
                 update_feats=True, update_coors=True, only_sparse_neighbors=False, valid_radius=float('inf'),
                 m_pool_method='sum', soft_edges=False, coor_weights_clamp_value=None):
        """
        Constructor method.
    ...
    def forward(self, feats, coors, edges=None, mask=None, adj_mat=None):
        ...
        b, n, d, device, fourier_features, num_nearest, valid_radius, only_sparse_neighbors = \
            (*feats.shape, feats.device, self.fourier_features, self.num_nearest_neighbors,
             self.valid_radius, self.only_sparse_neighbors)
...
class EGNN_Network(nn.Module):
    ...
    def __init__(self, *, depth, dim, num_tokens=None, num_edge_tokens=None, num_positions=None, edge_dim=0,
                 ...
                 global_linear_attn_dim_head=64, num_global_tokens=4, **kwargs):
        """
        ...
        Args:
            ...
            global_linear_attn_heads:
            global_linear_attn_dim_head:
            num_global_tokens:
            kwargs:
        """
    ...
    def forward(self, feats, coors, adj_mat=None, edges=None, mask=None, return_coor_changes=False):
        ...
        if exists(self.pos_emb):
            n = feats.shape[1]
            assert n <= self.num_positions, (f'given sequence length {n} must be less than the number of '
                                             f'positions {self.num_positions} set at init')
